...
class App
  ...
  constructor: ->
    ...
    @$referenceScreen = $ ".reference-screenshot"
    @$spinner = $ ".spinner"
    @$nameTag = $ ".name-tag"
    @$groupTag = $ ".group-tag"
    @$result = $ ".result"
  ...
  getName: (forceUpdate) ->
    groupName = (not forceUpdate and localStorage["groupName"]) || prompt "What's group name?"
    ...
    joinGroup(groupName, {
      ...
      group = @group.data();
      @$referenceScreen.attr('style', "background-image:url(" + group.assetUrl + ");")
      @$groupTag.text(group.name);
      @$spinner.hide();
    )
  ...
  saveContent: =>
    ...
    updateUser(@group, @user,
      score: @currentStreak,
      html: localStorage["content"],
    )
